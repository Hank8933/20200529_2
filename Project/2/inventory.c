...
void updatePrice(part inventory[], int num_parts);
    ...
    for (;;) {
        ...
        switch (code) {
            ...
            case 'u': update(inventory, num_parts);
                      break;
            case 'v': updatePrice(inventory, num_parts);
                      break;
...
void updatePrice(part inventory[], int num_parts)
{
 ...
 {
    struct part c = *(struct part *)a;
    struct part d = *(struct part *)b;
    if(c.number < d.number) {return -1;}
    else if (c.number == d.number) {return 0;}
    else return 1;
}
...
void print(part inventory[], int num_parts)
{
    qsort((void *)inventory, num_parts, sizeof(*inventory), cmp);
    int i;
